...
//Objects about basic sword
//buglist 5
ObjectID woodSword;
bool foundSword = false;

// Objects about store
ObjectID item0, item1, item2, item3, item4, item5,item6, item7, item8, item9;
...
//buglist 5
//Check if items are used sold or not
bool item0Use = false;
bool item1Use = false;
bool item2Use = false;
bool item3Use = false;
...
//buglist5
void useAttackButton(void);
void useItemButton(void);
void useBothButton(void);
...
void gameInit() {
	...
	tower3F_Scene = createScene(" towerLevel1", "./Images/Backgrounds/Tower_Inside.png");
	battle3F_Scene = createScene("zombie", "./Images/Backgrounds/Battle.png");
	tower4F_Scene = createScene("towerLevel2", "./Images/Backgrounds/Tower_Inside.png");
	...
	//basic sword(basic option)
	/*buglist 5
	woodSword = createObject("./Images/UI/Battle/WoodSword.png");
	locateObject(woodSword, tower3F_Scene, 200, 155);
	scaleObject(woodSword, 0.25f);
	showObject(woodSword);
	*/
...
void useItemButton() {//items used when you push item button
	//buglist 5
...
void useBothButton() {//items used when you attack or item button
	//buglist 5
...
void mouseCallback(ObjectID object, int x, int y, MouseAction action) {
	...
	else if (object == woodSword) {
		if (foundSword == false) {
			showMessage("나무검을 발견했다!");
			foundSword = true;
		}
		else {
			pickObject(woodSword);
		}
	}
	*/ 
	...
	//buglist 5(modify showMessage to korean)
	else if (object == merchant) {
		showMessage("이 상점에서 타워를 올라갈 때 필요한 아이템들을 살 수 있습니다");
	}
	//buglist 5(delete item 10, 11, 12)
...
void keyboardCallback(KeyCode code, KeyState state) {
	...
	if (code == 83) {				// RIGHT
		...
		if (currentScene == villageLeftScene) {
			...
			else if ((playerX > playerX_MAX - 100) && state == KeyState::KEYBOARD_PRESSED) {
				// buglist 4
				currentScene = villageScene;
		...
		if (currentScene == villageScene) {
			if (playerX != 100 && (playerX< playerX_MIN + 100) && state == KeyState::KEYBOARD_PRESSED) {
				// buglist 4
				currentScene = villageLeftScene;
				enterScene(currentScene);
			...
			else if ((200 < playerX) && (playerX < 300)) {
